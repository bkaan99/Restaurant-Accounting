  ...
  return (
    <section className={`${panelClass} ${dm ? "border-white/10 bg-white/5" : "border-indigo-100 bg-gradient-to-br from-white via-indigo-50/20 to-white"}`}>
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-indigo-400">İşlemler</p>
          <h2 className={`mt-1 text-lg font-semibold ${dm ? "text-slate-100" : "text-slate-900"}`}>Gelir / Gider Akışı</h2>
          <p className={`mt-1 text-xs ${dm ? "text-slate-400" : "text-slate-500"}`}>Sistemdeki tüm finansal hareketler tek ekranda</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className={`rounded-full border px-3 py-1 text-xs font-medium transition ${
              filter === "all"
                ...
                : dm ? "border-white/10 bg-white/5 text-slate-300 hover:bg-white/10" : "border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100"
            }`}
          >
            Tümü
          </button>
          ...
          <div className={`flex items-center gap-1 rounded-full border px-2 py-1 ${dm ? "border-white/10 bg-white/5" : "border-slate-200 bg-white"}`}>
            <select
              value={pageSize}
              onChange={(e) => { setPageSize(Number(e.target.value) as 15 | 30 | 50); setPage(1); }}
      ...
      {showFilters ? (
        <div className={`mb-4 flex flex-wrap items-end gap-2 rounded-2xl border px-3 py-2.5 shadow-sm ${dm ? "border-white/10 bg-white/5" : "border-indigo-100 bg-white"}`}>
          {["Bugün", "Son 7 Gün", "Son 30 Gün"].map((label, i) => (
            <button
              key={label}
              onClick={() => setQuickRange([1, 7, 30][i])}
              className={`rounded-full border px-2.5 py-1 text-xs font-medium transition ${dm ? "border-white/10 bg-white/5 text-slate-300 hover:bg-white/10" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-100"}`}
            >
              {label}
            </button>
          ))}
          <div className="ml-auto flex flex-wrap items-center gap-2">
            <div>
              ...
              <input
                ...
                className={`rounded-xl border px-2.5 py-1.5 text-xs outline-none focus:border-indigo-400 ${dm ? "border-white/10 bg-white/10 text-slate-200" : "border-slate-300 bg-white text-slate-600"}`}
              />
            </div>
            <div>
              ...
              paginatedRows.map((row) => (
                <Fragment key={row.id}>
                  <tr
                    ...
                    className={`cursor-pointer border-b transition ${dm ? "border-white/5 hover:bg-white/5" : "border-slate-100 hover:bg-indigo-50/40"}`}
                  >
                    <td className={`px-3 py-2 ${dm ? "text-slate-300" : "text-slate-700"}`}>{row.title}</td>
                    <td className={`px-3 py-2 ${dm ? "text-slate-400" : "text-slate-500"}`}>{row.reference}</td>
                    <td className={`px-3 py-2 ${dm ? "text-slate-400" : "text-slate-500"}`}>{row.dateText}</td>
                    <td className="px-3 py-2">
                      <span className={`rounded-full border px-2 py-0.5 text-[11px] font-medium ${dm ? "border-white/10 bg-white/5 text-slate-300" : "border-slate-200 bg-slate-50 text-slate-600"}`}>
                        {row.owner}
                      </span>
                    </td>
                    <td className={`px-3 py-2 text-right font-semibold ${row.type === "income" ? "text-emerald-500" : "text-rose-400"}`}>
                      {row.type === "income" ? "+" : "-"}{tl.format(row.amount)}
                    </td>
                  </tr>
                  {expandedRowId === row.id ? (
                    <tr className={`border-b ${dm ? "border-white/5 bg-white/5" : "border-slate-100 bg-slate-50/70"}`}>
                      <td colSpan={5} className="px-3 py-3">
                        {row.type === "income" ? (
                          <div>
                            <p className={`mb-2 text-xs font-semibold uppercase tracking-[0.12em] ${dm ? "text-slate-400" : "text-slate-500"}`}>Sipariş İçeriği</p>
                            <div className="space-y-1.5">
                              {row.items.map((item) => (
                                <div key={`${row.id}-${item.menuItemId}-${item.name}`} className={`flex items-center justify-between rounded-xl border px-2.5 py-1.5 text-xs ${dm ? "border-white/10 bg-white/5" : "border-slate-200 bg-white"}`}>
                                  <span className={dm ? "text-slate-300" : "text-slate-700"}>{item.name} x {item.qty}</span>
                                  <span className={`font-semibold ${dm ? "text-slate-100" : "text-slate-800"}`}>{tl.format(item.lineTotal)}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                          ...
                          <div>
                            <p className={`mb-1 text-xs font-semibold uppercase tracking-[0.12em] ${dm ? "text-slate-400" : "text-slate-500"}`}>Gider Detayı</p>
                            <p className={`text-xs ${dm ? "text-slate-300" : "text-slate-600"}`}>{row.note || "Bu gider için not bulunmuyor."}</p>
                          </div>
                        )}
                      </td>
                    </tr>
